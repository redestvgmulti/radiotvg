...
const AdminUsers = () => {
  ...
  const createUser = async () => {
    ...
    if (res.error || res.data?.error) {
      ...
      setNewEmail(''); setNewPassword(''); setShowCreate(false); fetchUsers();
    }
    setCreating(false);
  };
          ...
          {showCreate && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-5">
              <div className="p-4 rounded-2xl bg-card border border-border space-y-3">
                ...
                <motion.button whileTap={{ scale: 0.98 }} onClick={createUser} disabled={creating || !newEmail || !newPassword}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] text-muted-foreground">Último login: {formatDate(u.last_sign_in_at)}</span>
                      </div>
                    </div>
                  </div>
